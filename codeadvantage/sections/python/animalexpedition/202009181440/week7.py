...
def move_snake():
    ...
    if distance(new_x, new_y, food.food.xcor(), food.food.ycor()) < SQUARE_SIZE:
        add_segment()
        food.update()
...
class Food:
    ...
    def __init__(self, border):
        ...
        self.food.setx(self.food.xcor() + SQUARE_SIZE/2 + (border["right"]-border["left"])*2/3)
        self.food.sety(self.food.ycor() + SQUARE_SIZE/2 + (border["bottom"]-border["top"])/2)
...
class Broadcast:
    def __init__(self):
        self.log = []
    def broadcast(code):
        self.log.append((code, timestamp))
